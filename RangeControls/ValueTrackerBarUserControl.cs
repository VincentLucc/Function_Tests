...
public partial class ValueTrackerBarUserControl : UserControl
{
    ...
    public void LoadConfig(int _iMinLimit, int _iMaxLimit, int _iValue, string sValueCaption = "Value:")
    {
        ...
        //Set text box
        ValueCalcEdit.Text = iCurrentValue.ToString();
    }
    ...
    private void ValueCalcEdit_EditValueChanged(object sender, EventArgs e)
    {
        if (!int.TryParse(ValueCalcEdit.Text, out int iValue))
        {
            //Set value back
            ValueCalcEdit.Text = iCurrentValue.ToString();
            return;
