...
namespace Property_Normal_212
{
    public partial class FormMain : XtraForm
    {
        ...
        {
            Debug.WriteLine($"Cell Value changed.{e.Value}");
            var student = pg1.SelectedObject as Student;
            ...
            if (sFieldName == nameof(Student.CheckBox))
            {
                student.CheckBox = false;
                propertyHelper.ReloadAll();//Refresh display
            }
            else if (sFieldName == nameof(Student.ToggleSwitch))
            {
                if (student.ToggleSwitch == true)
                {
                    MessageBox.Show("Notice");
                    student.ToggleSwitch = false;
                    propertyHelper.ReloadAll(); //Refresh display
                }
            }
        }
