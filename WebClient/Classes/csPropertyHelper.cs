        ...
        private void PropertyGrid_CustomRecordCellEdit(object sender, GetCustomRowCellEditEventArgs e)
        {
            ...
            switch (editorInfo.EditorType)
            {
                ...
                case _editorType.SerialPort:
                    if (UniqueEditors.ContainsKey(editorInfo.EditorType))
                    {
                        RepositoryItemLookUpEdit lookupEditor = (RepositoryItemLookUpEdit)UniqueEditors[editorInfo.EditorType];
                        var portNames = SerialPort.GetPortNames();
                        lookupEditor.DataSource = portNames;
                        lookupEditor.DropDownRows = portNames.Length;
                        e.RepositoryItem = lookupEditor;
                    }
                    else
                    {
                        RepositoryItemLookUpEdit repositoryMacList = new RepositoryItemLookUpEdit();
                        repositoryMacList.ShowFooter = false;//Hide "X" button in bottom
                        var portNames = SerialPort.GetPortNames();
                        repositoryMacList.DataSource = portNames;
                        repositoryMacList.DropDownRows = portNames.Length;
                        repositoryMacList.TextEditStyle = TextEditStyles.DisableTextEditor;
