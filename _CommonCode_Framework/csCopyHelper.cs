...
namespace _CommonCode_Framework
{
    public static class csCopyHelper
    {
        ...
        /// <param name="copyMode">Copy [All Data]/[Marked Data]</param>
        /// <param name="handleTypeDifference">Allow to copy possible value when the data type doesn't match</param>
        /// <returns></returns>
        public static List<csInstanceCopyLog> CopyInstanceValues<tThis, tCopySource>(this tThis selfInstance, tCopySource copyInstance, _instanceCopyMode copyMode = _instanceCopyMode.None, bool handleTypeDifference = true)
        {
            var copyingData = new csInstanceCopingData() { CopyMode = copyMode, HandleTypeDifference = handleTypeDifference };
            "InstanceCopy.Start".TraceRecord();
            bool isSuccess = selfInstance.CopyValues(copyInstance, ref copyingData);
            string sComplete = $"InstanceCopy.Complete:[Count:{copyingData.PropertyChanges.Count}],[Success:{isSuccess}]";
            if (selfInstance != null)
            {
                var instanceType = selfInstance.GetType();
                sComplete += $",[Type:{instanceType.FullName}]";
            }
            sComplete.TraceRecord();
        ...
        private static void CopyFields(object selfInstance, object copySourceInstance, ref csInstanceCopingData copingData)
        {
            try
            {
                ...
                foreach (FieldInfo thisFieldInfo in thisFields)
                {
                    ...
                    if (thisFieldInfo.IsInitOnly || thisFieldInfo.IsLiteral) continue;
